...
Starting with `v0.1.9`, Engrams utilizes **Alembic** for automated database schema management. This means:

*   **Seamless Upgrades (for v0.1.9 users):** If you are updating from `v0.1.9`, Alembic will generally handle database schema changes automatically when the Engrams server starts.
*   **Data Preservation:** Alembic migrations are designed to preserve your existing data during schema upgrades.
*   **No Manual Intervention (Typically for v0.1.9 users):** For most updates from `v0.1.9`, simply updating the `context-portal-mcp` package and restarting the Engrams server is all that's required.
...
*   **Database Migration is Per-Workspace:** Each `context.db` file is independent. If a specific workspace's `context.db` requires a manual migration (i.e., it's from `v0.1.8` or earlier, or automatic migration failed), you **must** perform the "Manual Data Migration" steps (Section 5) *for that specific workspace*.
    *   **Exporting Data:** When exporting data (Section 5, Step 2), ensure you are in the correct workspace and use a unique `output_path` for each workspace's backup (e.g., `"engrams_backup_project_A"`, `"engrams_backup_project_B"`).
...
1.  **STOP ALL ENGRAMS PROCESSES IMMEDIATELY AND COMPLETELY:**
...
4.  **Restart the Engrams Server:** Start the Engrams server. Alembic should automatically apply any necessary schema updates.

5.  **Verify:** Check your Engrams logs (`logs/engrams.log`) for any database-related errors. If you encounter issues, proceed to Section 5.
...
1.  **STOP ALL ENGRAMS PROCESSES IMMEDIATELY AND COMPLETELY:**
    This is the most critical step. Before proceeding, ensure the Engrams server process is completely stopped. If it's running in a terminal, press `Ctrl+C`. If it's running as a background process (e.g., in STDIO mode without a visible terminal), you **must** identify and terminate its process.
    *   **On Windows (Command Prompt/PowerShell):**
...
2.  **Export Existing Data:**
    Before proceeding, you need to export all your current Engrams data to markdown files. This will create a directory (default: `engrams_export/`) containing markdown representations of your Product Context, Active Context, Decisions, Progress, System Patterns, and Custom Data.

    **How to export:**
    Ask your AI assistant (e.g., Roo Code, CLine) to export your Engrams data. You can use a natural language prompt like:
    "Please export my Engrams data to a backup directory named `engrams_backup`."
    ...
    ```xml
    <use_mcp_tool>
    <server_name>engrams</server_name>
    <tool_name>export_engrams_to_markdown</tool_name>
    <arguments>
    {
      "workspace_id": "${workspaceFolder}",
      "output_path": "engrams_backup"
    }
    </arguments>
    </use_mcp_tool>
    ```
...
7.  **Restart the Engrams Server:**
    Start the Engrams server again. Since `context.db` no longer exists, Alembic will automatically create a new database with the latest schema defined by the installed `context-portal-mcp` version.

8.  **Import Data Back:**
    Once the new database is created, you can re-import your saved data into it.

    **How to import:**
    Ask your AI assistant to import your Engrams data from the backup directory you created earlier. You can use a natural language prompt like:
    "Please import my Engrams data from the `engrams_backup` directory."

    If your AI assistant requires a specific command, it might look something like this (though your assistant should handle this for you):
    ```xml
    <use_mcp_tool>
    <server_name>engrams</server_name>
    <tool_name>import_markdown_to_engrams</tool_name>
    <arguments>
    {
      "workspace_id": "${workspaceFolder}",
      "input_path": "engrams_backup"
    }
    </arguments>
    </use_mcp_tool>
    ```
