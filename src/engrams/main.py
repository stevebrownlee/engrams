...
try:
    ...
    from .core.backward_compat import (
        detect_and_migrate_old_conport,
        get_workspace_with_fallback
    ...
    from src.engrams.core.backward_compat import (
        detect_and_migrate_old_conport
    )
...
async def engrams_lifespan(_server: FastMCP) -> AsyncIterator[None]:
    ...
    try:
        ...
        log.info("Engrams FastMCP server lifespan shutting down. Closing all DB connections.")
...
engrams_mcp = FastMCP(
    name="Engrams", # Pass name directly
    lifespan=engrams_lifespan
)
...
async def tool_log_decision(
    ...
    scope_id: Annotated[Optional[int], Field(
        description="Governance scope ID this item belongs to"
    )] = None,
    visibility: Annotated[Optional[str], Field(
...
async def tool_create_scope(
    ...
    scope_type: Annotated[str, Field(
        description="Type of scope: 'team' or 'individual'"
    )],
    scope_name: Annotated[str, Field(
        min_length=1, description="Human-readable name for the scope"
    )],
    created_by: Annotated[str, Field(
        min_length=1, description="Who is creating this scope (e.g., username)"
    )],
    parent_scope_id: Annotated[Optional[int], Field(
        description="Parent scope ID (for individual scopes under a team scope)"
    )] = None,
) -> Dict[str, Any]:
    """
...
async def tool_get_scopes(
    ...
    scope_type: Annotated[Optional[str], Field(
        description="Optional filter by scope type: 'team' or 'individual'"
    )] = None,
) -> List[Dict[str, Any]]:
    """
...
async def tool_log_governance_rule(
    ...
    rule_definition: Annotated[Dict[str, Any], Field(
    ...
    try:
        ...
        raise exceptions.ContextPortalError(f"Invalid arguments for log_governance_rule: {e}")
    except Exception as e:
        log.error(
            "Error processing args for log_governance_rule: %s. Args: workspace_id=%s, scope_id=%s",
            e, workspace_id, scope_id
        )
        raise exceptions.ContextPortalError(
...
async def tool_get_governance_rules(
    ...
    scope_id: Annotated[int, Field(
        description="Scope to get rules for (references context_scopes.id)"
    )],
    entity_type: Annotated[Optional[str], Field(
...
async def tool_check_compliance(
    ...
    item_type: Annotated[str, Field(
        description="Entity type to check (e.g., 'decision', 'system_pattern')"
    )],
    item_id: Annotated[int, Field(
        description="Entity ID to check compliance for"
    )],
) -> Dict[str, Any]:
    """
...
async def tool_get_scope_amendments(
    ...
    scope_id: Annotated[Optional[int], Field(
        description="Optional filter by scope ID"
    )] = None,
) -> List[Dict[str, Any]]:
    """
...
async def tool_review_amendment(
    ...
    amendment_id: Annotated[int, Field(
        description="ID of the amendment to review"
    )],
    status: Annotated[str, Field(
        description="Review decision: 'accepted' or 'rejected'"
    )],
    reviewed_by: Annotated[str, Field(
    ...
    try:
        ...
        raise exceptions.ContextPortalError(f"Invalid arguments for review_amendment: {e}")
    except Exception as e:
        log.error(
            "Error processing args for review_amendment: %s. Args: workspace_id=%s, amendment_id=%s",
            e, workspace_id, amendment_id
        )
        raise exceptions.ContextPortalError(
...
async def tool_bind_code_to_item(
    ...
    item_type: Annotated[str, Field(
        description="Engrams entity type (e.g., 'decision', 'system_pattern')"
    )],
    item_id: Annotated[int, Field(
        description="ID of the Engrams entity"
    )],
    file_pattern: Annotated[str, Field(
        min_length=1, description="Glob or path pattern (e.g., 'src/auth/**/*.py')"
    )],
    binding_type: Annotated[str, Field(
        description="Nature of the binding: 'implements', 'governed_by', 'tests', 'documents', 'configures'"
    )],
    symbol_pattern: Annotated[Optional[str], Field(
    ...
    try:
        ...
        raise exceptions.ContextPortalError(f"Invalid arguments for bind_code_to_item: {e}")
    except Exception as e:
        log.error(
...
async def tool_get_context_for_files(
    ...
    file_paths: Annotated[List[str], Field(
        description="List of file paths being edited (relative to workspace root)"
    )],
    binding_type_filter: Annotated[Optional[str], Field(
...
async def tool_verify_bindings(
    ...
    item_type: Annotated[Optional[str], Field(
...
async def tool_get_stale_bindings(
    ...
    days_stale: Annotated[int, Field(
        description="Number of days since last verification to consider stale"
    )] = 30,
) -> List[Dict[str, Any]]:
    """
...
async def tool_unbind_code_from_item(
    ...
    binding_id: Annotated[int, Field(
        description="ID of the code binding to remove"
    )],
) -> Dict[str, Any]:
    """
...
async def tool_get_relevant_context(
    ...
    task_description: Annotated[str, Field(
        min_length=1, description="Description of the current task for relevance scoring"
    )],
    token_budget: Annotated[Union[int, str], Field(
    ...
    scope_id: Annotated[Optional[Union[int, str]], Field(
        description="Optional scope ID for filtering (from governance feature)"
    )] = None,
    ...
    try:
        ...
        raise exceptions.ContextPortalError(f"Invalid arguments for get_relevant_context: {e}")
    except Exception as e:
        log.error(
            "Error processing args for get_relevant_context: %s. "
            "Args: workspace_id=%s, task_description='%s', token_budget=%s",
            e, workspace_id, task_description, token_budget
        )
        raise exceptions.ContextPortalError(
...
async def tool_estimate_context_size(
    ...
    try:
        ...
        log.error(
            ...
            e, workspace_id, task_description
        )
        raise exceptions.ContextPortalError(
...
async def tool_get_project_briefing(
    ...
    token_budget: Annotated[Optional[Union[int, str]], Field(
    ...
    scope_id: Annotated[Optional[Union[int, str]], Field(
        description="Optional governance scope ID for filtering (Feature 1)"
    )] = None,
) -> Dict[str, Any]:
    """
...
async def tool_get_briefing_staleness(
    ...
    stale_threshold_days: Annotated[Optional[Union[int, str]], Field(
        description="Days after which data is considered stale (default: 30)"
    )] = 30,
) -> Dict[str, Any]:
    """
...
async def tool_get_section_detail(
    ...
    section_id: Annotated[str, Field(
        description="Section ID to drill into (e.g., 'key_decisions', 'patterns')"
    )],
    token_budget: Annotated[Optional[Union[int, str]], Field(
        description="Optional max token budget for the section detail"
    )] = None,
    scope_id: Annotated[Optional[Union[int, str]], Field(
        description="Optional governance scope ID for filtering"
    )] = None,
) -> Dict[str, Any]:
    """
...
def main_logic(sys_args=None):
    ...
    if args.mode == "http":
        log.info("Starting Engrams HTTP server (via FastMCP) on %s:%s", args.host, args.port)
