...
class CodeBinding(BaseModel):
    ...
    symbol_pattern: Optional[str] = Field(None, description="Optional function/class name pattern")
    binding_type: str = Field(..., description="'implements', 'governed_by', 'tests', 'documents', 'configures'")
    confidence: str = Field(default="manual", description="'manual', 'agent_suggested', 'auto_detected'")
...
class CodeBindingVerification(BaseModel):
    ...
    id: Optional[int] = None
    binding_id: int
    verification_status: str = Field(..., description="'valid', 'file_missing', 'symbol_not_found', 'pattern_empty'")
...
class GetContextForFilesArgs(BaseArgs):
    ...
    binding_type_filter: Optional[str] = Field(None, description="Filter by binding type")
