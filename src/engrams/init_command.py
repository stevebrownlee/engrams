...
def _create_engrams_dirs(project_dir: Path) -> None:
    """Creates the .engrams/ directory structure for team projects."""
    engrams_dir = project_dir / ".engrams"
    for subdir in ("decisions", "patterns", "shared-data"):
        (engrams_dir / subdir).mkdir(parents=True, exist_ok=True)


def _write_config_seed(project_dir: Path, visibility: str) -> None:
    """
    Writes a config seed file that the MCP server reads on first tool call
    to seed engrams_config/default_decision_visibility into the DB.

    The seed file is at .engrams/config_seed.json.
    """
    seed_path = project_dir / ".engrams" / "config_seed.json"
    seed_path.parent.mkdir(parents=True, exist_ok=True)

    seed_data = {
        "default_decision_visibility": visibility,
        "_comment": f"Auto-generated by 'engrams init'. Visibility default: {visibility}",
    }
    with open(seed_path, "w") as f:
        json.dump(seed_data, f, indent=2)


def _prompt_team_or_solo() -> str:
    """Interactively asks the user if this is a team or solo project."""
    print()
    print("Project classification:")
    print("  [1] Team project — decisions shared via .engrams/ for git sync")
    print("  [2] Solo project — decisions stay in local database only")
    print()
    while True:
        try:
            choice = input("Choose [1/2] (default: 1 for team): ").strip()
        except (EOFError, KeyboardInterrupt):
            print("\nDefaulting to team project.")
            return "team"
        if choice in ("", "1"):
            return "team"
        elif choice == "2":
...
def _create_and_seed_database(project_dir: Path, visibility: str) -> Optional[Path]:
    """
    Creates the Engrams database and seeds the default visibility config.

    This is called during ``engrams init`` to proactively create the database
    so it's ready for the LLM's first conversation.

    Args:
        project_dir: The project root directory (used as workspace_id).
        visibility: The default decision visibility ('team' or 'individual').

    Returns:
        The Path to the created database, or None if creation failed.
    """
    workspace_id = str(project_dir.resolve())

    try:
        from .db.database import create_database, get_db_connection, close_db_connection
        from .db import models
        from datetime import datetime, timezone

        # Create the database (directory, alembic files, migrations)
        db_path = create_database(workspace_id)

        # Seed the default_decision_visibility directly into the DB
        conn = get_db_connection(workspace_id)
        cursor = conn.cursor()
        import json as _json

        now = datetime.now(timezone.utc)
        value_json = _json.dumps(visibility)
        cursor.execute(
            """
            INSERT OR REPLACE INTO custom_data (timestamp, category, key, value)
            VALUES (?, ?, ?, ?)
            """,
            (now, "engrams_config", "default_decision_visibility", value_json),
        )
        conn.commit()
        cursor.close()

        # Clean up the connection (the MCP server will create its own later)
        close_db_connection(workspace_id)

        log.info(
            "Database created and seeded at %s (visibility=%s)",
            db_path,
            visibility,
        )
        return db_path
        ...
        log.warning("Failed to create database during init: %s", e)
        print(
            f"  ⚠ Database creation error: {e}",
            file=sys.stderr,
        )
        return None
...
def init_strategy(
    ...
    if team:
        ...
        # Interactive prompt
        visibility = _prompt_team_or_solo()

    # Write config seed for the MCP server (fallback for non-init setups)
    _write_config_seed(target_dir, visibility)

    if visibility == "team":
        _create_engrams_dirs(target_dir)
        print(f"  → Created .engrams/ directory structure for team sync")
        print(f"  → All decisions will be automatically shared via .engrams/ for git sync.")
    else:
        print(f"  → Decisions will be stored locally in your Engrams database.")

    print(f"  → Default visibility: {visibility}")

    # --- Create the database proactively ---
    db_path = _create_and_seed_database(target_dir, visibility)
    if db_path:
        print(f"  → Database initialized at {db_path}")
    else:
        print(
            f"  ⚠ Database creation failed. The MCP server will auto-create it "
            f"on first use.",
            file=sys.stderr,
        )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    classification.add_argument(
        "--team",
        action="store_true",
        help="Configure as a team project. Decisions are shared via .engrams/ for git sync.",
    )
    classification.add_argument(
        "--solo",
        action="store_true",
        help="Configure as a solo project. Decisions stay in the local database only.",
    )
