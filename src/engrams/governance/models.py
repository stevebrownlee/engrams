...
class GovernanceRule(BaseModel):
    ...
    rule_type: str = Field(..., description="'hard_block', 'soft_warn', 'allow_with_flag'")
    entity_type: str = Field(..., description="Which entity type this rule governs")
    rule_definition: Dict[str, Any] = Field(..., description="Structured rule definition (JSON)")
...
class ScopeAmendment(BaseModel):
    ...
    target_item_id: int
    status: str = Field(..., description="'proposed', 'under_review', 'accepted', 'rejected'")
...
class CreateScopeArgs(BaseArgs):
    ...
    scope_type: str = Field(..., description="'team' or 'individual'")
    scope_name: str = Field(..., min_length=1, description="Human-readable name for the scope")
    parent_scope_id: Optional[int] = Field(None, description="FK to parent scope (for individual under team)")
...
class GetScopesArgs(BaseArgs):
    ...
    scope_type: Optional[str] = Field(None, description="Filter by scope type ('team' or 'individual')")
...
class LogGovernanceRuleArgs(BaseArgs):
    ...
    rule_type: str = Field(..., description="'hard_block', 'soft_warn', or 'allow_with_flag'")
    entity_type: str = Field(..., description="Entity type this rule governs")
    rule_definition: Dict[str, Any] = Field(..., description="Structured rule definition")
    ...
    def validate_rule_type(self) -> 'LogGovernanceRuleArgs':
        if self.rule_type not in ('hard_block', 'soft_warn', 'allow_with_flag'):
            raise ValueError("rule_type must be 'hard_block', 'soft_warn', or 'allow_with_flag'")
        return self
...
class GetEffectiveContextArgs(BaseArgs):
    ...
    scope_id: int = Field(..., description="Individual scope ID to get effective context for")
