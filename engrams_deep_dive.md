# Context Portal (Engrams) MCP Server Documentation
...
*   **Workspace-Specific Context:** All data managed by Engrams is tied to a `workspace_id` (typically the absolute path to a project directory). This ensures that context remains relevant to the specific project being worked on.
...
*   **Structured Data Management:** Engrams defines several core data entities (see Section 2) to structure project knowledge, such as decisions, progress, system patterns, and custom data.
*   **Tool-Based Interaction:** AI assistants interact with Engrams by calling its defined MCP tools, each designed for a specific operation (e.g., logging a decision, retrieving active context).
*   **Knowledge Graph Construction:** Engrams facilitates the creation of a project-specific knowledge graph by storing structured entities (decisions, code patterns, glossary terms) and allowing explicit, queryable relationships to be defined between them using tools like `link_engrams_items`.
*   **Vector Embeddings & Semantic Search:** Engrams generates and stores vector embeddings for key text content within various entities, enabling semantic similarity search.
...
Engrams utilizes an SQLite database, specific to each workspace, to store its structured context. The key data entities and their corresponding database tables are:
...
7.  **Context Links (`context_links`)**
    *   **Purpose:** Establishes explicit, queryable relationships between different Engrams data entities (e.g., a decision is implemented by a system pattern, a progress item tracks a decision), forming the edges of the **project knowledge graph**.
    *   **Schema:**
        *   `id` (INTEGER, PK, AUTOINCREMENT)
...
9. **Vector Store (ChromaDB)**
    *   **Purpose:** Stores vector embeddings generated from text content of various Engrams entities (Decisions, Progress, Custom Data, etc.) to enable semantic similarity search.
    *   **Location:** Stored on disk within the workspace's Engrams data directory, separate from the SQLite file but managed alongside it.
...
*   **Arguments:**
    ...
    *   `linked_item_type` (string, optional): Optional: Type of the Engrams item this progress entry is linked to (e.g., 'decision', 'system_pattern') (Default: null)
    *   `linked_item_id` (string, optional): Optional: ID/key of the Engrams item this progress entry is linked to (requires linked_item_type) (Default: null)
...
#### 3.7.1 `export_engrams_to_markdown`
*   **Description:** Arguments for exporting Engrams data to markdown files.
*   **Arguments:**
    *   `workspace_id` (string): Identifier for the workspace (e.g., absolute path) (Required: Yes)
...
#### 3.8.1 `link_engrams_items`
*   **Description:** Arguments for creating an explicit, typed link between two Engrams items, thereby enriching the **project knowledge graph**.
*   **Arguments:**
    *   `workspace_id` (string): Identifier for the workspace (e.g., absolute path) (Required: Yes)
...
The server code is primarily located within the `src/engrams/` directory:
...
This mode is intended for direct use by local MCP clients (like IDE extensions), and the `FastMCP` library handles the STDIO communication transport.
    *   Command (from project root):
        ```bash
        python src/engrams/main.py --mode stdio --workspace_id "/path/to/your/workspace"
        ```
    *   Or using the CLI entry point:
        ```bash
        engrams --mode stdio --workspace_id "/path/to/your/workspace"
        ```
...
The server can also be run as an HTTP service using Uvicorn:
    ```bash
    uvicorn src.engrams.main:app --host 127.0.0.1 --port 8000
    ```
Or using the CLI entry point:
    ```bash
    engrams --mode http --host 127.0.0.1 --port 8000
    ```
...
`<workspace_id>/engrams/context.db`
...
3.  **Register Tool in `main.py`:**
    ...
    *   Use the `@engrams_mcp.tool(name="your_new_tool_name")` decorator on a new async wrapper function.
...
4.  **Update `TOOL_ARG_MODELS`:** Add your new tool name and its argument model to the `TOOL_ARG_MODELS` dictionary in `db/models.py` (used by `get_engrams_schema`).
...
*   Real-time updates/notifications if a multi-user model were adopted for the same Engrams instance.
...
### 5.4 Enhanced Retrieval for RAG (Retrieval-Augmented Generation)
Engrams functions as a key component in a RAG system by enabling the construction and querying of a **project-specific knowledge graph**. Its capabilities, now including **vector embeddings** and **semantic search**, serve as the "Retrieval" mechanism.

*   **Vector Embeddings & Semantic Search:** Engrams integrates vector embeddings for text content within various entities (Decisions, Progress, Custom Data, System Patterns). This allows for finding conceptually similar items beyond keyword matches using the `semantic_search_engrams` tool, significantly improving the quality of retrieved context for LLM augmentation. This is powered by a dedicated vector store (ChromaDB) managed alongside the SQLite database.
...
*   **Context Chunking for LLMs:** (Future Consideration) Introduce a tool or mechanism to retrieve specific Engrams data items (or parts of them, like a long decision rationale) in "chunks" optimized for LLM context windows. This would help in feeding manageable pieces of relevant information to the LLM.
*   **Automated Context Suggestion:** (Future Consideration) Develop tools within Engrams that, based on a query or a summary of the current task (perhaps provided by the LLM), could proactively suggest relevant Engrams items (decisions, patterns, glossary terms) that the LLM might want to retrieve to augment its generation. This could use a combination of FTS, semantic search, and knowledge graph traversal (`get_linked_items`).
...
*   While STDIO is primary for local IDE integration, developing an alternative HTTP-based communication mode (potentially using Server-Sent Events for streaming if applicable) could broaden Engrams's accessibility to web-based clients or other tools that prefer HTTP over STDIO. This would leverage the existing FastAPI framework.

### 5.8 Flexible Database Storage & Identification
*   Building upon the 'Configuration & Usability' point for `workspace_id`, implement the 'Flexible Database Storage (DatabaseID)' model (Decision ID 52). This would allow users to store Engrams databases in locations outside the workspace root (e.g., a centralized local directory or true remote storage). It involves assigning a unique `DatabaseID` to each workspace's context, which clients use for identification, while the Engrams server maps this ID to the actual database location and connection details. The local workspace path would still be relevant for resolving file references within the context data.
...
These considerations depend on the evolving goals and use cases for the Engrams server. The current architecture provides a solid foundation for many of these potential enhancements.

## Prompt Caching Integration

Engrams plays a crucial role in enabling AI assistants to effectively utilize **prompt caching** with compatible LLM providers (such as Google Gemini, Anthropic Claude, and OpenAI). By providing structured, queryable project context, Engrams supplies the stable, frequently used information that forms the basis for caching.

**How Engrams Supports Prompt Caching:**

1.  **Structured Context as Cacheable Content:** Engrams stores key project information (Product Context, System Patterns, Custom Data like glossaries or specifications) in a structured database. This allows AI assistants to reliably retrieve specific, often large, blocks of text that are ideal candidates for inclusion in the cacheable prefix of prompts sent to LLMs.
...
3.  **AI Assistant Strategy Guidance:** The custom instruction files for AI assistants (like those in `engrams-custom-instructions/`) are updated to include a `prompt_caching_strategies` section. This section, which can be included directly in the prompt or referenced from a central file (`engrams/prompt_caching_strategy.yml`), guides the AI on:
    *   Identifying suitable cacheable content from Engrams based on heuristics and user hints.
...
The AI assistant, upon receiving a user request, identifies necessary context from Engrams using tools like `get_product_context`, `get_custom_data`, etc. Based on the retrieved content and the defined prompt caching strategy, the assistant constructs the prompt sent to the LLM API. For providers with implicit caching, the cacheable Engrams content is placed at the beginning. For providers with explicit caching, the content is included with the necessary markers.

This integration ensures that the valuable, structured knowledge stored in Engrams is actively used to improve the efficiency and cost-effectiveness of AI interactions through prompt caching.